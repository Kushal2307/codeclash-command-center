...
const UpwardSnow = () => {
  ...
  useEffect(() => {
    ...
    const colors = ["#00f2fe", "#8a2be2", "#00f2fe", "#00f2fe"];
    ...
    const animate = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        // Glow
        ctx.beginPath();
        ctx.arc(p.x, p.y, Math.max(currentSize * 2.5, 1), 0, Math.PI * 2);
        ctx.fillStyle = p.color;
        ctx.globalAlpha = Math.max(currentOpacity * 0.15, 0);
        ctx.fill();

        if (p.y < -10 || currentOpacity <= 0) {
